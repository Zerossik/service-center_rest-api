...
Якщо ви все зробили вірно, то на email користувача буде відправлений лист з
...
Це другий крок, але він не обов'язковий. Цей крок необхідний, якщо ви хочете
відображати компонент за умовою, наприклад, якщо токен валідний, то показуйте
користувачу форму для введення нового паролю, якщо не валідний, то покажіть
помилку! Отримує body з полями `token, id` - token та id вам були відправлені в
посиланні на пошту в першому запиті.
...
Якщо ви все зробили вірно, то пароль успішно буде замінено на новий, а
користувач отримає на пошту email.
...
Наприклад:
`https://service-center-6fck.onrender.com/api/contacts?page=1&limit=10`.  
Якщо не передати параметри пагінації, то сервер поверне 20 контактів за
замовчуванням.
...
Наприклад:
`https://service-center-6fck.onrender.com/api/contacts?filter=samsung`  
Сервер поверне всі документи, які відповідають ключовому запиту.

---

### `GET https://service-center-6fck.onrender.com/api/contacts/archive - get all Archive`
...
Для пагінації необхідно передати параметри **page** та **limit**.  
Наприклад:
`https://service-center-6fck.onrender.com/api/contacts/archive?page=1&limit=10`.  
Якщо не передати параметри пагінації, то сервер поверне 20 контактів за
замовчуванням.

Для фільтрування за типом, необхідно передати параметр **type**  
Наприклад:
`https://service-center-6fck.onrender.com/api/contacts/archive?type=phone`  
Сервер поверне всі записи з типом `phone`.  
Для пошуку за ключовим словом, необхідно передати параметр **filter**  
Наприклад:
`https://service-center-6fck.onrender.com/api/contacts/archive?filter=samsung`  
Сервер поверне всі документи, які відповідають ключовому запиту.
...
Отримує body, обов'язкові поля: **type, manufacturer, model, customerName,
phoneNumber, failure**.  
Необов'язкові поля: **deviceID, price, status, masterName, endDate,
description**.  
example:
...
Якщо все успішно, сервер поверне 201 код та об'єкт з доданим майстром! Інакше
поверне помилку.
...
Якщо все успішно, сервер поверне 200 код та об'єкт з видаленим майстром! Інакше
поверне помилку.
...
Запит на оновлення виробника, отримує body с полями
...
Якщо тип був успішно видалений, то сервер відправе успішну відповідь, інакше -
помилку.
...
При успішному створенні або оновленні сервер поверне оновленний об'єкт
userSettings!
